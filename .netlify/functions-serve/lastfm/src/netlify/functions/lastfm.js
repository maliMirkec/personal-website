...
function cloneNode(node, recursive = false) {
  ...
  if (isText(node)) {
    ...
    const clone3 = new Element(node.name, __spreadValues({}, node.attribs), children2);
    children2.forEach((child) => child.parent = clone3);
    if (node.namespace != null) {
      clone3.namespace = node.namespace;
    }
    if (node["x-attribsNamespace"]) {
      clone3["x-attribsNamespace"] = __spreadValues({}, node["x-attribsNamespace"]);
    }
    if (node["x-attribsPrefix"]) {
      clone3["x-attribsPrefix"] = __spreadValues({}, node["x-attribsPrefix"]);
    }
    result = clone3;
...
var fromCodePoint = (_a4 = String.fromCodePoint) !== null && _a4 !== void 0 ? _a4 : function(codePoint) {
  let output = "";
  if (codePoint > 65535) {
    codePoint -= 65536;
...
function findAll(test, nodes) {
  ...
  while (elem = stack.shift()) {
    const children2 = (_a5 = elem.children) === null || _a5 === void 0 ? void 0 : _a5.filter(isTag2);
...
function cloneDom(dom) {
  const clone3 = "length" in dom ? Array.prototype.map.call(dom, (el) => cloneNode(el, true)) : [cloneNode(dom, true)];
  const root2 = new Document(clone3);
  clone3.forEach((node) => {
    node.parent = root2;
  });
  return clone3;
...
var import_css_what5 = __toModule(require_commonjs());

// node_modules/css-select/lib/esm/index.js
var import_boolbase6 = __toModule(require_boolbase());

// node_modules/css-select/lib/esm/compile.js
var import_css_what4 = __toModule(require_commonjs());
var import_boolbase5 = __toModule(require_boolbase());

// node_modules/css-select/lib/esm/sort.js
var import_css_what = __toModule(require_commonjs());
...
var import_css_what2 = __toModule(require_commonjs());
...
var subselects = {
  ...
  has(next2, subselect, options, _context, compileToken2) {
    ...
    const context = subselect.some((s2) => s2.some(isTraversal)) ? [PLACEHOLDER_ELEMENT] : void 0;
...
var import_css_what3 = __toModule(require_commonjs());
...
function includesScopePseudo(t2) {
  return t2.type === import_css_what4.SelectorType.Pseudo && (t2.name === "scope" || Array.isArray(t2.data) && t2.data.some((data2) => data2.some(includesScopePseudo)));
}
...
function convertOptionFormats(options) {
  ...
  (_a5 = opts.adapter) !== null && _a5 !== void 0 ? _a5 : opts.adapter = esm_exports;
...
function getLoad(parse8, render3) {
  return function load2(content, options, isDocument2 = true) {
    ...
    function initialize(selector, context, root2 = initialRoot, opts) {
      ...
      const r2 = typeof root2 === "string" ? [parse8(root2, options2, false, null)] : "length" in root2 ? root2 : [root2];
...
var Parser2 = class {
  ...
  emitOpenTag(name) {
    ...
    (_d2 = (_c = this.cbs).onopentagname) === null || _d2 === void 0 ? void 0 : _d2.call(_c, name);
  ...
  onclosetag(start, endIndex) {
    ...
    if (!this.isVoidElement(name)) {
      ...
      (_d2 = (_c = this.cbs).onopentag) === null || _d2 === void 0 ? void 0 : _d2.call(_c, "br", {}, true);
  ...
  onend() {
    ...
    if (this.cbs.onclosetag) {
      ...
      for (let i2 = this.stack.length; i2 > 0; this.cbs.onclosetag(this.stack[--i2], true))
        ;
    }
...
// netlify/functions/lastfm.mjs
exports.handler = async function(event) {
  return await fetch("https://www.last.fm/user/maliMirkec").then((response) => response.text()).then((data2) => {
    const $2 = load(data2);
    let tracks = [];
    $2(".chartlist-row--with-artist").each((i2, row) => {
      tracks.push({
        track: $2(row).find(".chartlist-name").text().trim(),
        artist: $2(row).find(".chartlist-artist").text().trim(),
        trackUrl: `https://www.last.fm${$2(row).find(".chartlist-name a").attr("href")}`
      });
    });
    return {
      statusCode: 200,
