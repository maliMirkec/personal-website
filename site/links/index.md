---
layout: default
title: My links
description: Here are my links and profiles from all over the web, from social networks to projects and publications.
type: default
sections:
  - type: hero
    title: Around the Web
    text: Here are my links and profiles from all over the web, from social networks to projects and publications.
    text_width: 40
sections2:
  - type: webrings
  - type: banner
    banner: consultation
    sticky: true
---

## Social networks

- [Mastodon](https://mastodon.social/@cita)
- [BlueSky](https://bsky.app/profile/silvestar.codes)
- [LinkedIn](https://www.linkedin.com/in/starbist/)
- [Reddit](https://www.reddit.com/user/starbist)
- [Last.fm](https://last.fm/user/maliMirkec)

## Development

- [GitHub](https://github.com/maliMirkec)
- [CodePen](https://codepen.io/CiTA/)
- [NPM](https://www.npmjs.com/~starbist)
- [VS Code](https://marketplace.visualstudio.com/publishers/starbist)

## Freelance

- [Toptal](https://www.toptal.com/resume/silvestar-bistrovic#trust-nothing-but-brilliant-freelancers)
- [Codementor](https://www.codementor.io/@malimirkeccita)
- [About.me](https://about.me/starbist)

## Projects

- [Advent of redesign](/side-projects/redesign/)
- [Invalid CSS](/side-projects/invalid-css/)
- [CSS Auditors](/side-projects/css-auditors/)
- [Code Line Daily](/side-projects/code-line-daily/)
- [UI Dev Mentoring](/side-projects/ui-dev-mentoring/)
- [Advent of UI components](/side-projects/advent/)
- [Starter Project](/side-projects/starter-project/)
- [Making a website with Hugo](/side-projects/hugo/)

## Other

- [My setup](/uses/)
- [My branding](/branding/)
- [My favorites](/favorites/)
- [What I don’t do?](/no-skills/)
- [What I don’t allow (on my site mostly)?](/no-ads/)

## Publications

- [Smashing Magazine: Infinite-Scrolling Logos In Flat HTML And Pure CSS](https://www.smashingmagazine.com/2024/04/infinite-scrolling-logos-html-css/)
- [Smashing Magazine: How To Build An Expandable Accessible Gallery](https://www.smashingmagazine.com/2021/10/build-expandable-accessible-gallery/)
- [CSS Tricks: How to Make a “Raise the Curtains” Effect in CSS](https://css-tricks.com/css-raise-the-curtains-effect/)
- [CSS Tricks: The CSS from-font Value Explained in 4 Demos](https://css-tricks.com/from-font-value-text-decoration-thickness/)
- [CSS Tricks: Tools for Auditing CSS](https://css-tricks.com/tools-for-auditing-css/)
- [CSS Tricks: How to Get Sticky and Full-Bleed Elements to Play Well Together](https://css-tricks.com/how-to-get-sticky-and-full-bleed-elements-to-play-well-together/)
- [CSS Tricks: A Comparison of Static Form Providers](https://css-tricks.com/a-comparison-of-static-form-providers/)
- [My Favorite Netlify Features](https://css-tricks.com/my-favorite-netlify-features/)
- [CSS Tricks: Oh, the Many Ways to Make Triangular Breadcrumb Ribbons!](https://css-tricks.com/oh-the-many-ways-to-make-triangular-breadcrumb-ribbons/)
- [Frontend Masters: Drawing a Line to Connect Elements with CSS Anchor Positioning](https://frontendmasters.com/blog/drawing-a-line-to-connect-elements-with-css-anchor-positioning/)
- [Frontend Masters: Building a Live Preview with Eleventy, Contentful, and Liquid Templating](https://frontendmasters.com/blog/building-a-live-preview-with-eleventy-contentful-and-liquid-templating/)
- [LogRocket blog: 5 things to consider when creating your CSS style guide](https://blog.logrocket.com/5-things-to-consider-when-creating-your-css-style-guide-7b85fa70039d/)
- [Toptal blog: Overview of Popular Static Site Generators](https://www.toptal.com/front-end/static-site-generators-comparison-2018)
- [Toptal blog: A Guide to Static Site Generators Using Hexo and WordPres](https://www.toptal.com/front-end/wordpress-to-html-with-hexo-blog)
- [Practical Dev](https://dev.to/starbist)
- [Medium](https://medium.com/@malimirkeccita)
- [Hacker Noon](https://hackernoon.com/@malimirkeccita)
